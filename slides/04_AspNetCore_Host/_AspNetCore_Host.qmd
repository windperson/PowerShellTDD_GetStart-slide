...
```csharp{.cs filename="src\DemoWebApi\Services\DuplicateFileFinderInvokeService.cs"}
var result =
    ps.Invoke(input: null, settings: psInvocationSettings)
        .FirstOrDefault()?.BaseObject as object[];
...
foreach (dynamic duplicateFileInfo in result)                             // <1>
{
    var duplicateFileResult = new DuplicateFileResult
    {
        SourceFilePath = duplicateFileInfo.FilePath1.ToString(),          // <2>
        CompareFilePath = duplicateFileInfo.FilePath2.ToString()          // <2>
    };
...
```
1. 使用 `dynamic` 關鍵字來簡化原本 PowerShell 程式碼中定義的自訂 class 型別。
2. 就可以直接使用物件的屬性值（C#編譯器不檢查，如果寫錯就會有 runtime exception）。
